...
def convert_num_to_bitfield(label_data, h, w, npz_name, root_folder, h5_file, cls_num=19):
    ...
    for n in label_list: # Iterate in each pixel
        ...
        bit_tensor = paddle.to_tensor(bitfield)
        actual_len = bit_tensor.size()[0]
        padded_bit_tensor = paddle.concat((paddle.zeros([cls_num-actual_len]).byte(), bit_tensor.byte()), dim=0)
        all_bit_tensor_list.append(padded_bit_tensor)
    #all_bit_tensor_list = paddle.stack(all_bit_tensor_list).view(h, w, cls_num)
    all_bit_tensor_list = paddle.reshape(paddle.stack(all_bit_tensor_list),shape=[h,w,cls_num])
