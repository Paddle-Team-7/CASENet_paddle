...
class CityscapesData(io.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for k in xrange(num_cls):
            if np_data[:,:,num_cls-1-k].sum() > 0: # The order is reversed to be consistent with class name idx in official.
                ...
                label_tensor = self.label_transform(paddle.to_tensor(np_data[:, :, num_cls-1-k]).unsqueeze(0).float())
            else: # ALL zeros, don't need transform, maybe a bit faster?..
                label_tensor = paddle.zeros([1, self.input_size, self.input_size]).float()
            label_data.append(label_tensor.squeeze(0).long())
        #运行label_data.shape 知道长度之后进行transpose
        label_data = paddle.stack(label_data).transpose(0,1).transpose(1,2) # N X H X W -> H X W X N
