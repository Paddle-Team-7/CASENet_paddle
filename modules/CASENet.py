...
def gen_mapping_layer_name(model):
    ...
    for k in list(layer_to_name_dict.keys()):
    ...
    for (m_name, m_data) in model.state_dict().items():
        if m_name in layer_to_name_dict:
            if not ("running_mean" in m_name or "running_var" in m_name):
                ...
                pretrained_dict[m_name] = paddle.to_tensor(np_data)
                ...
                pretrained_dict[m_name] = paddle.to_tensor(up_data/down_data)
                print("{0} loaded successfully".format(m_name))

    # Store model into pth file.
    model.load_state_dict(pretrained_dict)
    paddle.save({'state_dict': model.state_dict()}, loaded_model_path)
...
def init_bilinear(arr):
    ...
    return paddle.to_tensor(weight.reshape(shape))
...
class ScaleLayer(nn.Layer):
    ...
    def __init__(self, size, init_value=0.001):
        ...
        #self.scale = nn.Parameter(torch.FloatTensor([init_value]*size)) #requires_grad=False
        xc = paddle.to_tensor([init_value]*size,dtype = 'float32')
        self.scale = paddle.create_parameter(shape=xc.shape, dtype=str(xc.numpy().dtype),
                        default_initializer=paddle.nn.initializer.Assign(xc))
        self.scale.stop_gradient = True

        #self.bias = nn.Parameter(torch.FloatTensor([init_value]*size))
        xb = paddle.to_tensor([init_value]*size,dtype = 'float32')
        self.bias = paddle.create_parameter(shape=xb.shape, dtype=str(xb.numpy().dtype),
                        default_initializer=paddle.nn.initializer.Assign(xb))
        self.bias.stop_gradient = True

    def forward(self, input_data):
        return input_data * self.scale.unsqueeze(0).unsqueeze(2).unsqueeze(3) + self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3)

class CropLayer(nn.Layer):
...
class ConcatLayer(nn.Layer):

    def __init__(self):
        super(ConcatLayer, self).__init__()

    def forward(self, input_data_list, dim):
        concat_feats = paddle.concat((input_data_list), dim=dim)
        return concat_feats
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, special_case=False):
        ...
        if special_case:
            self.conv2 = nn.Conv2D(planes, planes, kernel_size=3,
                                   dilation=4, padding=4,bias_attr=False)
        else:
            self.conv2 = nn.Conv2D(planes, planes, kernel_size=3,
                               dilation=2, padding=2,bias_attr=False)
...
class ResNet(nn.Layer):
    ...
    def __init__(self, block, layers, num_classes=19):
        ...
        self.conv1 = nn.Conv2D(3, 64, kernel_size=7, stride=1, padding=3,bias_attr=False)
        self.bn_conv1 = nn.BatchNorm2D(64,momentum=0.1)
        ...
        self.score_edge_side1 = nn.Conv2D(64, 1, kernel_size=1, bias_attr=True)
        self.score_edge_side2 = nn.Conv2D(256, 1, kernel_size=1, bias_attr=True)
        self.upsample_edge_side2 = nn.Conv2DTranspose(1, 1, kernel_size=4, stride=2, bias_attr=False)
        set_require_grad_to_false(self.upsample_edge_side2)
        
        self.score_edge_side3 = nn.Conv2D(512, 1, kernel_size=1, bias_attr=True)
        self.upsample_edge_side3 = nn.Conv2DTranspose(1, 1, kernel_size=8, stride=4, bias_attr=False)
        set_require_grad_to_false(self.upsample_edge_side3) 
        
        self.score_cls_side5 = nn.Conv2D(2048, num_classes, kernel_size=1, bias_attr=True)
        self.upsample_cls_side5 = nn.Conv2DTranspose(num_classes, num_classes, kernel_size=16, stride=8, groups=num_classes, bias_attr=False)
        set_require_grad_to_false(self.upsample_cls_side5) 

        self.ce_fusion = nn.Conv2D(num_classes*4, num_classes, kernel_size=1, groups=num_classes, bias_attr=True)
        ...
        self.upsample_edge_side2.weight.set_value(init_bilinear(self.upsample_edge_side2.weight))
        self.upsample_edge_side3.weight.set_value(init_bilinear(self.upsample_edge_side3.weight))
        self.upsample_cls_side5.weight.set_value(init_bilinear(self.upsample_cls_side5.weight))
...
if __name__ == "__main__":
    ...
    input_data = paddle.rand([2, 3, 472, 472])
